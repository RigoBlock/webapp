...
//TODO: move exchange and asset select and find address in separate component
...
export default class ActionDragoFinalize extends Component {
  ...
  onSend = () => {
    ...
    instance.finalizeDealCFDExchange
      .estimateGas(options, values)
      .then((gasEstimate) => {
        options.gas = gasEstimate.mul(1.2).toFixed(0);
        console.log(`finalize: gas estimated as ${gasEstimate.toFixed(0)} setting to ${options.gas}`);

        return instance.finalizeDealCFDExchange.postTransaction(options, values);
      })
      .then(() => {
        this.props.onClose();
        this.setState({
